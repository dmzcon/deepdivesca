    ...
    if res == 0:
        ...
        if grep_res == 0:
            ...
            if tar_res == 0:
                ...
                dda_modulefor_db_interaction.DDA_DBSmartInsert_Component(who,dbh,ossname,purl,"pypi",version)
            else:
    ...
    if res == 0:
        ...
        if grep_res == 0:
            ...
            if tar_res == 0:
                ...
                SaveWorkdirToDB(conveyor_id,workdirpath)
                SetDwnLdrStatus(conveyor_id,"Success")
                dda_modulefor_db_interaction.DDA_DBSmartInsert_Component(who,dbh,ossname,purl,"npm",version)
            else:
                SetDwnLdrStatus(conveyor_id,"Failure-TAR")
        else:
            SetDwnLdrStatus(conveyor_id,"Failure-GREP")
...
def     MainJobSubroutine():
    ...
    sth = dbh.cursor()
    sth.execute("select purl_brief,version,id from factor_handler_conveyor where dwnldr_status='ToDo' and purl_brief LIKE 'pkg:%' order by id asc")
