...
def     MainJobSubroutine():
    ...
    sth = feedlog.cursor()
    sth.execute("select purl,version,count(purl) from log.requests where (event_time + '10000 minute') > now() group by purl,version order by count(purl) desc")
    ...
    while 1:
        ...
        if row[1] is None:
            ...
            vers = row[1]
        AnalyzePurlStringRecord(row[0],vers)
        index = index+1
    sth.close()
